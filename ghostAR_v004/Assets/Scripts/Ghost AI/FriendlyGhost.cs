...
public class FriendlyGhost : MonoBehaviour, IGhost, IPlayerHitable {
    ...
    /// <summary>
    /// The SkinnedMeshRenderer attached to the friendly.
    /// </summary>
    [Tooltip("The SkinnedMeshRenderer attached to the friendly.")]
    public SkinnedMeshRenderer ghostRenderer;
    /// <summary>
    /// The Animator attached to the Friendly Ghost
    /// </summary>
    [Tooltip("The Animator attached to the Friendly Ghost.")]
    ...
    /// <summary>
    /// The AudioManager within the scene.
    /// </summary>
    ...
    void Start () {
        ...
        healPop = transform.GetChild(1).GetComponent<AudioSource>();
        stopClip = transform.GetChild(2).GetComponent<AudioSource>();
        leftClip = transform.GetChild(3).GetComponent<AudioSource>();
        rightClip = transform.GetChild(4).GetComponent<AudioSource>();
    ...
    /// <summary>
    /// Picks the direction the Friendly will roam in randomly.
    /// </summary>
    ...
    /// <summary>
    /// Checks the Fade status of the Friendly
    /// </summary>
    ...
    /// <summary>
    /// Restricts certain values so that they do not go over or under they're desired values.
    /// </summary>
    ...
    /// <summary>
    /// The behavior that happens when the player hits this friendly.
    /// </summary>
                ...
                if (giveHealth) { //Give the player health
                    ...
                    if (referenceGhost != null) {
                        if (Mathf.Sign(distanceFromHostile) == 1) {         //Play Left
                            ghostAnimator.SetTrigger("Say Left");
                            leftClip.Play();
                        } else if (Mathf.Sign(distanceFromHostile) == -1) { //Play Right
                            ghostAnimator.SetTrigger("Say Right");
                            rightClip.Play();
                        }
                    } else {
                        ghostAnimator.SetTrigger("Say Stop");
                        stopClip.Play();
                    }
                }
