...
public class ComboSystem : MonoBehaviour {
    #region Variables
    /// <summary>
    /// The Jumpscare within the scene.
    /// </summary>
    [Tooltip("The Jumpscare within the scene.")]
    public JustWalker jWalk;
    /// <summary>
    ...
    void ComboDisplayManagement() {
        ...
        if (ComboScore >= 5) {
            ...
            ChangeAlphaInDisplays(0.86f);
        } else if (ComboScore == 3) {
            ChangeAlphaInDisplays(0.72f);
        } else if (ComboScore == 2) {
            ChangeAlphaInDisplays(0.58f);
        } else if (ComboScore == 1) {
            ChangeAlphaInDisplays(0.44f);
        } else if (ComboScore <= 0) {
            ChangeAlphaInDisplays(0.3f);
        }
    }
    ...
    public void IncreaseComboByOne() {
        ...
        if (ComboScore == 10 || ComboScore == 20 || ComboScore == 30 || ComboScore == 40 || ComboScore == 50) {
            jWalk.ScarePlayer();
        }
        timeLeft = comboTime;
        EvaluateBestCombo();
